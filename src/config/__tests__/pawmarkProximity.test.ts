 ...
 *        → 2026-08-23 재조정: 공원 30 / 산책로·해변 50 / 강변 100 / 나머지 10
...
describe('발도장 허용 반경', () => {
  it('공원 30 / 산책로·해변 50 / 강변 100', () => {
    expect(getPawmarkRadius('park')).toBe(30);
    expect(getPawmarkRadius('trail')).toBe(50);
    expect(getPawmarkRadius('beach')).toBe(50);
    expect(getPawmarkRadius('riverside')).toBe(100);
  });

  it('시설(카페·병원·미용·호텔)과 쉼터·기타는 10m로 좁다 — 옆 가게에서 찍히면 안 된다', () => {
    for (const c of ['pet_cafe', 'vet', 'pet_grooming', 'pet_boarding', 'rest_spot', 'other'] as const) {
      expect(getPawmarkRadius(c)).toBe(10);
    }
  });

  it('모르는 카테고리는 가장 좁은 기본값을 쓴다', () => {
    expect(getPawmarkRadius(undefined)).toBe(PAWMARK_PROXIMITY.DEFAULT_RADIUS_M);
    expect(PAWMARK_PROXIMITY.DEFAULT_RADIUS_M).toBe(10);
  });
...
describe('정확도 마진', () => {
  ...
  it('공원 실질 허용치는 최대 50m다 (30 + 마진 상한 20)', () => {
    expect(getPawmarkRadius('park') + margin(9999)).toBe(50);
  });

  it('시설 실질 허용치는 최대 30m다 (10 + 마진 상한 20)', () => {
    expect(getPawmarkRadius('vet') + margin(9999)).toBe(30);
  });
