/**
 * ⚠️ SEC-05: 이 함수는 프로덕션에서 **410 스텁으로 중화(비활성)** 되어 있다.
 *    이 파일은 그 스텁이 아니라 **중화 이전의 옛 구현**이다 — 그대로 배포하면 취약점이 되살아난다.
 *
 *    사유: 토큰의 앱 소속(aud)을 검증하지 않고 email 기준 magiclink OTP를 내주므로
 *          다른 앱에서 받은 카카오 토큰으로 남의 계정을 가져갈 수 있었다.
 *    현재: 앱은 이 브리지를 쓰지 않는다. `supabase.auth.signInWithIdToken({ provider: 'kakao' })`
 *          로 이전했다(app/(auth)/login.tsx). 즉 이 함수는 호출되지 않는다.
 *    재개할 일이 생기면: audience 검증을 먼저 붙일 것. (naver-auth도 같은 상태 — SEC-07)
 *
 * ── 이하 옛 구현 (보존용) ────────────────────────────────────────
 *
